...
import { ENV, source_code_pro } from '../../../../utils'
import { useAcordionToogle, useData } from '../../../../hooks'
...
export const MenuCategorias = ({ toogleMegaMenu }: Props) => {
  ...
  const { data: menuData, isLoading } = useData<ICategory>(
  ...
  const selectedCategory = menuData?.data.find(
      ...
      <div tw="container">
              ...
              {menuData?.data.map(category => (
                <li key={category.id}>
                  <Link
                    ...
                    onMouseEnter={() =>
                      handleSelection(category.id, 'category')
                    }
                  >
        ...
        <div tw="2xl:hidden">
          {isLoading ? (
            ...
            <>
              {menuData?.data.map(categorieCatalog => (
